...
Now, as the author of the book (or its maker), most of your time developing the story will be spent inside the `manuscript` folder. In this directory you'll see a subfolder for each page of the book with a number appended to its foldername, like so: `page-1`, `page-2`, `page-3`… and so on until page-2N.
